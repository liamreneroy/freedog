...
class MotorControl:
    ...
    def sin_rollpitchyaw(self, publish_hz=200, set_bpm=None, sleep_override=None, loop_repeats=2, 
                        rollpitchyaw_array=np.array([0, 0, 0]),
        ...
        set_bpm = [0, 200] (bpm - integer) -> Sets the bpm at which the loops of the robot execute. If nothing is set, the robot will move
        ...
        if set_bpm:
            ...
            if set_bpm <= 0:
                raise ValueError("set_bpm must be greater than 0")

            while set_bpm > 100:
                set_bpm /= 2 

            bpm_to_bps_factored = (60. / set_bpm)                    # Added the divide by 2 so that the robot doesn't move too quickly
            sleep_rate = (1 / int(publish_hz)) * bpm_to_bps_factored 
