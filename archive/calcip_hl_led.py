...
while True:
    ...
    if motiontime % 100 == 0: #Print every 50 cycles
    ...
    if(motiontime >= 2000 and motiontime < 4000):
        hcmd.mode = MotorModeHigh.IDLE
        hcmd.led = led(255,0,0)
        if(motiontime == 2000):
            print(f">>> LED RED ON <<<\n")
    ...
    if(motiontime >= 4000 and motiontime < 6000):
        hcmd.mode = MotorModeHigh.IDLE
        hcmd.led = led(0,255,0)
        if(motiontime == 4000):
            print(f">>> LED GREEN ON <<<\n")

    if(motiontime >= 6000 and motiontime < 8000):
        hcmd.mode = MotorModeHigh.IDLE
        hcmd.led = led(0,0,255)
        if(motiontime == 6000):
            print(f">>> LED BLUE ON <<<\n")

    if(motiontime >= 8000 and motiontime < 10000):
        hcmd.mode = MotorModeHigh.IDLE
        hcmd.led = led(255,255,255)
        if(motiontime == 8000):
            print(f">>> LED ALL ON <<<\n")

    if(motiontime >= 10000 and motiontime < 12000):
        hcmd.mode = MotorModeHigh.IDLE
        hcmd.led = led(0,0,0)
        if(motiontime == 10000):
            print(f">>> LED ALL OFF <<<\n")
