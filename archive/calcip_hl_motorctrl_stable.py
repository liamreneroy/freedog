...
class MotorControl:
    ...
    def change_bpm(self, new_bpm):
        ...
        # Set limiter based on control function ~ sets max BPM range [0, bpm_limiter]. 
        ...
        while self.bpm > bpm_limiter:       # Sets motion limiter
            self.bpm /= 2. 
    ...
    def sin_euler_ctrl(self, mode='default', publish_hz=200, bpm=30, 
        ...
        sin_func_array = [math.sin, udcf.neg_sin, ... ] -> Continuous functions from math or user_defined 'udcf' library.
        ...
        self.oscillating_functions = [math.sin, udcf.neg_sin, udcf.abs_sin, udcf.neg_abs_sin]
...
# Have one check function that checks all values. 
#   Each has an if function to only check if it exists
#   Use for all generic ctrl checks 
# Create individual check functions for specific values for specific ctrl checks



### Old Code:
# # Set up the phase array based on the control function (sin or cos)
# if self.sin_func_array == math.cos:
#     for index_012 in range(0,3):
#         self.phase_array[index_012] = self.period_array[index_012] / -4.0

# # Check if phase is valid for sin() function
# if self.sin_func_array == math.sin and self.force==False:
#     for index_012 in range(0,3):
